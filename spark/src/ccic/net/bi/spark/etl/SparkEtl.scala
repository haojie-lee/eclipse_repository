...
object SparkEtl {
  ...
  //private final val MASTER_URL = "hdfs://master1:8020" //for test
  private final var MASTER_URL = ""  // for prod


  //System.setProperty("hadoop.home.dir","J:\\ProgramData\\Program Files\\hadoop-2.7.3")
  //System.setProperty("log4j.rootLogger",s"ERROR,console")
  ...
  /**
    ...
    *              add if not exists partition(workdate=20180710);
  ...
  def insertPartition(sql: String){

    try {
      val sqlArr = sql.split(";")  //

      for (i <- 0 until sqlArr.length){
        println(sdf.format(new Date) + " | 执行SQL:" + sqlArr(i))
        if (sqlArr(i).trim.isEmpty || sqlArr(i).trim.equals("")){
          println(sdf.format(new Date) + " | SQL语句为空，不执行。")
        }else{
          spark.sql(sqlArr(i))
        }
      }
  ...
  def processParquet(sql: String, outputName: String, saveMode: String) {
    ...
    if ("0".equals(saveMode)) { //覆盖''
      try {
        ...
        spark.sql(s"DROP TABLE ${tablename}_tmp_view")
      } catch {
      ...
      try {
        ...
        outData.write.mode(SaveMode.Overwrite).saveAsTable(outputName + "_tmp")
        spark.sql(s"INSERT INTO TABLE ${outputName} SELECT * FROM ${outputName}_tmp")
