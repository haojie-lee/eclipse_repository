...
object MergeBitFiles {
  ...
  def main(args: Array[String]): Unit = {
   ...
   //参数为3个，处理分区表全表
    ...
    if (overWrite.toLowerCase.equals("false")) {
      ...
      //参数为四个处理普通表数据
      val partition_flag=args(3)
      if(partition_flag.equals("no_partition_table")){
        merge(db, tbName, overWrite, partition_flag)
      }
  ...
  //合并普通表数据
//  def mergetable(db:String,tbName:String,overwrite:String){
//     println("=====merge table bit files:" + db + "." + tbName)
//      val appName = "MegerBitFiles-" + db + "." + tbName + "-" + "common table"
